...
nrows = 1 #len(df['event_title'].unique())
ncols = 1
figsize = (6.3 * ncols, 3.2 * nrows)
fig, axes = plt.subplots(nrows=nrows, ncols=ncols, figsize=figsize, squeeze=False)

col_idx = 0
for row_idx, event_title in enumerate([titles['image']]):
    ...
    ax.legend(title='Classifier', ncol=1, loc='best', handles=h, labels=l,
...
fig.savefig(os.path.join(output_path, 'mean_acc_allChannels_selection.pdf'), bbox_inches='tight')
plt.close(fig)
