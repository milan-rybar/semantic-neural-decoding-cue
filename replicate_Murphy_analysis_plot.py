...
nrows = len(df['classifier_name'].unique())
ncols = len(df['component_order'].unique())
figsize = (6.3 * ncols, 3.2 * nrows)
fig, axes = plt.subplots(nrows=nrows, ncols=ncols, figsize=figsize, squeeze=False)

for col_idx, component_order in enumerate(sorted(df['component_order'].unique())):

    for row_idx, classifier_title in enumerate(sorted(df['classifier_title'].unique())):
...
fig.savefig(os.path.join(output_path, f'boxplot.png'), bbox_inches='tight')
plt.close(fig)
