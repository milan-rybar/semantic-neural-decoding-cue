...
vmin = min([df['score'].min() for df in dfs])
vmax = max([df['score'].max() for df in dfs])
...
fig.savefig(os.path.join(output_path, 'P2_AGG_boxplot_allChannels.pdf'), bbox_inches='tight')
plt.close(fig)
