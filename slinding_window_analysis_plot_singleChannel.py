...
for event_name in df['event_name'].unique():
    ...
    _df = df[df['event_name'] == event_name]
    ...
    for col_idx, time_end in enumerate(sorted(_df['time_end'].unique())):
        for row_idx, classifier_name in enumerate(sorted(_df['classifier_name'].unique())):
            ...
            if len(df_selection) > 0:
                ax.set_title('{} {:.1f}-{:.1f} ms'.format(df_selection['classifier_name'].unique()[0],
                                                          df_selection['time_start'].unique()[0] * 1000,
    ...
    fig.savefig(os.path.join(output_path, f'scalpmap_{event_name}.png'), bbox_inches='tight')
    plt.close(fig)
